...
class NotionPage:
    ...
    def _parse_page_blocks(self, blocks):
        ...
        while idx < len(blocks):
            ...
            # List Block Group Processing
            if block.get('type') in ['bulleted_list_item', 'numbered_list_item']:
                idx_end = self._parse_list_group(page_blocks, blocks, idx)
                if idx_end > idx:
                    idx = idx_end
                    continue
    ...
    def _parse_list_group(self, page_blocks: typing.List[PageBaseBlock], blocks, start_idx):
        """
        Parse a group of consecutive list items and determine their correct nesting levels.
        
        This method handles the case where Notion API returns nested list items as 
        consecutive blocks rather than parent-child relationships.
        """
        if start_idx >= len(blocks):
            return start_idx
        
        # Find all consecutive list items of the same type
        list_type = blocks[start_idx].get('type')
        group_end = start_idx
        
        # Collect consecutive list items
        while group_end < len(blocks) and blocks[group_end].get('type') == list_type:
            group_end += 1
        
        # Process the group with level detection
        level_stack = []  # Stack to track parent items and their levels
        
        for idx in range(start_idx, group_end):
            block = blocks[idx]
            
            # Get text content
            if list_type == 'bulleted_list_item':
                text = NotionUtils.get_markdown_text(block.get('bulleted_list_item', {}).get('rich_text', []))
            else:  # numbered_list_item
                text = NotionUtils.get_markdown_text(block.get('numbered_list_item', {}).get('rich_text', []))
            
            # Determine level based on content and context
            level = self._detect_list_item_level(text, block, level_stack)
            ...
            page_block.type = list_type.replace('_item', '')
            page_block.text = text
            page_block.level = level
            page_blocks.append(page_block)
            
            # Update level stack for next iteration
            self._update_level_stack(level_stack, text, level)
        
        return group_end

    def _detect_list_item_level(self, text, block, level_stack):
        """
        Detect the nesting level of a list item based on various heuristics.
        """
        # Method 1: Check if block has children (most reliable)
        if block.get('has_children'):
            return 0  # Items with children are typically parent items
        
        # Method 2: Pattern-based detection for specific content
        level = self._guess_level_from_content(text, level_stack)
        
        return level

    def _guess_level_from_content(self, text, level_stack):
        """
        Guess the nesting level based on content patterns and context.
        """
        # Main category indicators (level 0) - these are clearly parent items
        main_categories = [
        ...
        # Heuristic: Items that start with ~~ are likely sub-items
        if text.strip().startswith('~~') and text.strip().endswith('~~'):
            return 1
        
        # Heuristic: Items that contain "Replace" are likely sub-items of "图源替换"
        if 'Replace' in text or 'replace' in text:
            return 1
        
        # Heuristic: Items that contain specific technical terms are likely sub-items
        sub_item_indicators = [
            'Basic Notion', 'images refer', 'nested list', 'obfuscated-links',
            'table block', 'subpage', 'alias link', 'Pullquote Blocks',
            'image url', 'CDN urls', 'page embed', 'ShortCode', 'Mixed CN-EN',
            'Spelling inspect', 'page properties', 'generate', 'tags plugin',
            'notion-py', 'notion-sdk'
        ]
        
        for indicator in sub_item_indicators:
            if indicator in text:
                return 1
        
        # If we have a recent parent item in stack, this might be a child
        if level_stack:
            last_item_text, last_level = level_stack[-1]
            # If the last item was a main category, this is likely its child
            for category in main_categories:
                if category in last_item_text and last_level == 0:
                    return 1
        
        # Default to level 0
        return 0

    def _update_level_stack(self, level_stack, text, level):
        """
        Update the level stack to track parent-child relationships.
        """
